...
def send_telegram_alert(message):
    ...
    if not token or not chat_id:
        print("⚠️ Alerta no enviada: Faltan variables de entorno TELEGRAM_BOT_TOKEN o TELEGRAM_CHAT_ID")
        return
...
def run_prediction_cycle():
    ...
    print(f"--- Iniciando Ciclo de Predicción para {SYMBOL} ---")
    ...
    for timeframe in TIME_FRAMES:
        ...
        if df is not None:
            ...
            if len(df) > 20: # Asegurar datos suficientes
                ...
                print(f"Modelo {timeframe} entrenado. Precisión: {accuracy_scores[timeframe]:.2%}")

    # 2. Verificación específica para la alerta de 15 minutos
    tf_alert = '15min'
    if tf_alert in trained_models:
        df_now = get_kucoin_klines(SYMBOL, tf_alert)
        if df_now is not None:
            df_now = add_technical_indicators(df_now).dropna()
            # Tomar la última fila para predecir
            ...
            direction = "Subir 📈" if proba_up >= proba_down else "Bajar 📉"
            ...
            if highest_proba > 0.40:
                ...
                msg = (f"🚨 *ALERTA DE PROBABILIDAD ALTA ({tf_alert})*\n\n"
                       f"💰 *Precio:* {precio:.8f}\n"
                       f"🔮 *Predicción:* El precio tiende a *{direction}*\n"
                       f"🎯 *Probabilidad:* {highest_proba:.2%}\n"
                       f"📊 *Confianza del Modelo:* {accuracy_scores[tf_alert]:.2%}\n"
                       f"⏰ *Hora Bogotá:* {bogota_now.strftime('%H:%M:%S')}")
                send_telegram_alert(msg)
            else:
                print("La probabilidad no supera el 70%. No se envía alerta.")
